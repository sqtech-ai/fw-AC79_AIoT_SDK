...
#if LWIP_IPV6
unsigned char netif_ip6_isobtained(const struct netif *netif);
#endif
    ...
    while (netif != NULL) {
        ...
        for (int i = 0; i < LWIP_IPV6_NUM_ADDRESSES; i++) {
            ...
            if (strcmp(ip6addr_ntoa(&netif->ip6_addr[i].u_addr.ip6), "::"))
                printf("ip6 address[%c%c%"U16_F"]: %s, type = %s\n",
...
#if LWIP_IPV6
        if (netif_ip6_isobtained(netif)) {
            //TODO:fixme
            printf("netif has obtained the IPv6 address, DHCPV4 timeout, ignore?");
            return;
        }
#endif
    ...
    if ((reason & LWIP_NSC_IPV6_ADDR_STATE_CHANGED) && (args != NULL)) {
        ...
        if (netif_ip6_addr_state(netif, addr_idx) & IP6_ADDR_PREFERRED)  {
            if (!ip6_addr_islinklocal(netif_ip6_addr(netif, addr_idx))\
                ...
                printf("IPV6 Addr[%c%c%"U16_F"]: %s, type: %s\n", netif->name[0], netif->name[1], (u16_t)netif->num, \
                       ip6addr_ntoa(netif_ip_addr6(netif, addr_idx)), \
                       ipv6_addr_types_to_str[get_ip6_addr_type(netif_ip6_addr(netif, addr_idx))]);

                if (netif == &wireless_netif) {
                    lwip_event_cb(NULL, LWIP_IP_GOT_IPV6_SUCC);
                } else if (netif == &lte_netif) {
                    lte_lwip_event_cb(NULL, LWIP_IP_GOT_IPV6_SUCC);
                }
            }
        }
    }
...
#if MULTI_NETIF_SUPPORT
    dns_servers_update_wifi_netif(netif);
#endif
}

void lwip_nd6_renew(u8_t lwip_netif)
{
    struct netif *netif = net_get_netif_handle(lwip_netif);
    if (netif == NULL) {
        printf("%s, not found netif\n");
        return;
    }

#if LWIP_IPV6
    if ((lwip_netif == WIFI_NETIF) && netif_ip6_isobtained(netif)) {
        nd6_renew(netif);
    }
#endif
}

void dns_servers_update(u8_t lwip_netif)
{
    struct netif *netif = net_get_netif_handle(lwip_netif);
    if (netif == NULL) {
        return;
...
void lwip_register_ext_wireless_netif(struct netif *netif)
{
    ext_netif = netif;
}

...
bool is_pcma_silence(const uint8_t *data, size_t length)
{
    const uint8_t SILENT_BYTE = 0xD5; // A-law静音编码值
...
//     const int DEFAULT_READ_SIZE = 40;
    /* const int DEFAULT_READ_SIZE = 160; */
// #endif
...
#if 1
    while (true && joined) {    // 发送音频数据，根据需要设置打断循环条件

 ...
 * @param[in,out] ctx MD5 context
 */
void tc_utils_md5_reset(IotMd5Context *ctx)
{
    memset(ctx, 0, sizeof(IotMd5Context));
    _utils_md5_init(ctx);
 ...
 */
void tc_utils_md5_update(IotMd5Context *ctx, const uint8_t *input, size_t ilen)
{
    size_t   fill;
    uint32_t left;
 ...
 * @param[in,out] ctx MD5 context
 */
void tc_utils_md5_finish(IotMd5Context *ctx)
{
    int      i;
    uint32_t last, padn;
    uint32_t high, low;
    ...
    tc_utils_md5_update(ctx, sg_iot_md5_padding, padn);
    tc_utils_md5_update(ctx, msglen, 8);
 ...
 */
int tc_utils_md5_compare(IotMd5Context *ctx, const char md5sum[33])
{

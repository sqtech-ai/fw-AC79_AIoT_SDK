...
typedef struct {
    ...
    void *mqtt_client;
    void *downloader;
    void *break_point_file_fp;
    void *firmware_file_fp;
...
static int _ota_break_point_init(void *usr_data)
{
    ...
    tc_utils_md5_reset(&handle->download_md5_ctx);
    memset(&handle->break_point, 0, sizeof(handle->break_point));
    if (!handle->break_point_file_fp) {
        handle->break_point_file_fp = HAL_FileOpen(OTA_BREAK_POINT_FILE_PATH, "rb+");
    }
    size_t len =
        HAL_FileWrite((uint8_t *)&handle->break_point, 1, sizeof(handle->break_point), handle->break_point_file_fp);
...
static int _ota_break_point_save(void *usr_data)
{
    ...
    tc_utils_md5_update(&handle->download_md5_ctx, handle->download_buff, handle->download_size);
    ...
    HAL_FileSeek(handle->break_point_file_fp, 0, SEEK_SET);
    size_t write_size =
        HAL_FileWrite((uint8_t *)&handle->break_point, 1, sizeof(handle->break_point), handle->break_point_file_fp);
...
static int _ota_break_point_restore(void *usr_data)
{
    ...
    while (size > 0) {
        ...
        if (!HAL_FileRead(handle->download_buff, 1, rlen, handle->firmware_file_fp)) {
            Log_e("read data failed");
            handle->break_point.downloaded_size = 0;
            break;
        }
        tc_utils_md5_update(&handle->download_md5_ctx, handle->download_buff, rlen);
        size -= rlen;
        total_read += rlen;
    }
    return 0;
...
{
    OTADownloaderHandle *handle = (OTADownloaderHandle *)usr_data;
    if (!handle->firmware_file_fp) {
        handle->firmware_file_fp = HAL_FileOpen(OTA_FILE_PATH, "wb+");
    }
    return handle->download_size > 0 ? HAL_FileWrite(handle->download_buff, 1, handle->download_size,
            handle->firmware_file_fp) != handle->download_size
           : 0;
}
...
static int _ota_data_download_finish(void *usr_data, UtilsDownloaderStatus status)
{
    ...
    case UTILS_DOWNLOADER_STATUS_SUCCESS:
        tc_utils_md5_finish(&handle->download_md5_ctx);
        // do something when ota finish;

        int valid = !tc_utils_md5_compare(&handle->download_md5_ctx, handle->download_now.md5sum);
...
void ota_downloader_info_set(OTAFirmwareInfo *firmware_info, const char *url, int url_len)
{
    if (OTA_DOWNLOADER_STATUS_DOWNLOADING != sg_ota_downloader_handle.status) {
        ...
        sg_ota_downloader_handle.download_percent = 0;
